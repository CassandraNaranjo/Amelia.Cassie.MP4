...
SELECT title.id, title.title, title.production_year, COUNT(cast_info.person_id) as Num, name.gender, ((count(cast_info.person_id))/115)*100 as ratio
from title
join cast_info
on title.id = cast_info.movie_id
join name
...
```{r}
ggplot(data = avengers, aes(x = gender, y = ratio)) +
